...
namespace Enhanced_ADS
{
	...
	internal class InputManager__SetAimInputUsingMouse
	{
		...
		static bool Prefix(InputManager __instance, Vector2 mouseDelta)
		{
			...
			if (__instance.characterMainControl.IsInAdsInput)
			{
				...
				)
				{
					...
					target_screen_pos.x = Mathf.Clamp(target_screen_pos.x, 0, Screen.width);
					target_screen_pos.y = Mathf.Clamp(target_screen_pos.y, 0, Screen.height);
			...
			if (__instance.characterMainControl.IsInAdsInput)
			{
				...
				{
					State.out_of_range.color = distance <= gun.BulletDistance * .5f + .5f
						? Color.white
						: distance <= gun.BulletDistance + .5f
							? new Color(1f, .5f, 0f)
							: Color.red;
					State.out_of_range.text = $"{System.Math.Round(distance, 1)}/{System.Math.Round(gun.BulletDistance * .5f, 1)}M";
		...
		static (float ex, float ey_up, float ey_dn, float max_x, float max_y_up, float max_y_dn) camera_bounds(float aim_range, Vector2 center, float edge_offset)
		{
			...
			float max_x = screen_offset_to_world(new Vector2(center.x, 0)).x * (aim_range - ex) / ex;
			float max_y_up = screen_offset_to_world(new Vector2(0, center.y)).y * (aim_range - ey_up) / ey_up;
			float max_y_dn = Mathf.Abs(screen_offset_to_world(new Vector2(0, -center.y)).y) * (aim_range - ey_dn) / ey_dn;
