...
`edge = (center.x - 15, center.y - 15)`. `State.delta` lerps 0→1 at `+= 3 * Time.deltaTime` (~0.33s). delta=0: ADS 진입 첫 프레임(카메라 패닝 없음). delta=1: 풀 카메라 패닝 활성.
...
### Trace_Aim_Point camera_offset formula
The `screen_offset_to_world(delta_from) - screen_offset_to_world(delta_to)` terms compensate for cursor pull toward center so the world aim direction stays continuous.
...
|---|---|---|---|
| `Adaptive_Sensitivity` | `center + world_offset_to_screen(ads_world_aim - camera_offset)` (≠ center) | `center + world_offset_to_screen(ads_world_aim)` 직접 사용 | 정확 |
| `Trace_Aim_Point` | `center + world_offset_to_screen(ads_world_aim * (1-delta))` | `center + world_offset_to_screen(ads_world_aim)` 직접 사용 | 정확 (delta 무관) |
| `Scrollable` | 물리적 커서 위치 | `aim_pos + world_offset_to_screen(camera_offset)` | 근사 |

조건: `ads_mode_type != Scrollable`이면 `center + world_offset_to_screen(ads_world_aim)` 사용 (코드상 `else` 분기의 첫 번째 if). `ads_world_aim`이 월드 공간이라 FOV 보정 불필요.

`Trace_Aim_Point`에서 이전 방식(`aim_pos += world_offset_to_screen(camera_offset)`)이 delta=1에서만 정확했던 이유: delta=1일 때만 aim_pos=center이므로 `center + world_offset_to_screen(camera_offset) = center + world_offset_to_screen(ads_world_aim)`. delta < 1이면 aim_pos = `center + world_offset_to_screen(ads_world_aim*(1-delta))` ≠ center이므로 `+= world_offset_to_screen(camera_offset)`은 비선형 오차 발생.

### State 변수 의미

- **`ads_world_aim`**: 카메라 피벗 기준 조준점의 XZ 월드 오프셋. 매 프레임 누적, ADS 종료 시 zero. Adaptive_Sensitivity에서는 커서 조준 + 카메라 패닝의 합산. Trace_Aim_Point에서는 카메라 목표점(커서는 따라옴).
- **`camera_offset`**: 카메라 XZ 패닝 오프셋(월드). `GameCamera__UpdateAimOffsetNormal`을 통해 실제 카메라에 적용. 유효 조준 범위를 커서 한계 너머로 확장. ADS 종료 시 zero.
- **`delta`**: 0→1 lerp 가중치. delta=0이면 카메라 고정(커서만으로 조준), delta=1이면 풀 패닝. `camera_offset = delta * ads_world_aim * ratio`.
...
**Trace_Aim_Point가 FOV 보정을 건너뛰는 이유**: `ads_world_aim`은 월드 공간이라 FOV 변경과 무관. `screen_offset_to_world(mouseDelta)`는 매 프레임 현재 FOV로 계산되므로 자동 반영. ADS 종료 복원도 `ads_world_aim` 직접 사용으로 FOV 독립적.
